...
public class Code39 implements BarcodeGenerator {
    ...
    public void generateBarcodes(File file, JLabel statusLabel, List<JCheckBox> selectedCheckboxes, boolean withName) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            File outputDir = new File("C:/Barcodes/Code 39/" + format);
    ...
    private int generateBarcodeForField(String barcodeText, String label, File outputDir, boolean withName) {
        int successCount = 0;
        try {
            // Dynamically calculate width: 30 pixels minimum + 15 pixels per character
            int dynamicWidth = Math.max(270, barcodeText.length() * 15);
            int height = 100;

            BitMatrix matrix = new MultiFormatWriter().encode(barcodeText.toUpperCase(), BarcodeFormat.CODE_39, dynamicWidth, height);
            ...
            if (withName) {
                ...
                int x = (barcodeImage.getWidth() - textWidth) / 2; // center text
