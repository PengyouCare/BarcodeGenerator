...
public class Code93 implements BarcodeGenerator {
    ...
    public void generator(File selectedFile, JLabel url, JComboBox<String> barName, JLabel validBarText, String nameFieldValidation) {
        ...
        code128.generator(selectedFile, url, barName, validBarText, nameFieldValidation);
    }
    ...
    public void generateBarcodes(File file, JLabel statusLabel, List<JCheckBox> selectedCheckboxes, boolean withName) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            File outputDir = new File("C:/Barcodes/Code 93/" + format);
            if (!outputDir.exists()) {
                outputDir.mkdirs();
            }
    ...
    private int generateBarcodeForField(String barcodeText, String label, File outputDir, boolean withName) {
        int successCount = 0;
        try {
            BitMatrix matrix = new MultiFormatWriter().encode(barcodeText, BarcodeFormat.CODE_93, 270, 100);
            ...
            // Create final image without left/right padding
            BufferedImage finalImage = new BufferedImage(width, height + labelHeight, BufferedImage.TYPE_INT_RGB);
            Graphics2D g = finalImage.createGraphics();
            g.setColor(Color.WHITE);
            g.fillRect(0, 0, finalImage.getWidth(), finalImage.getHeight());

            // Draw barcode at top-left (no padding)
